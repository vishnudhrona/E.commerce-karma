...
let userHome = async (req, res) => {
    try {
        ...
        let { products, totalProducts } = await productHelpers.getAllProducts(page, limit);
...
let changeProductQuantity = (req, res, next) => {
    userHelpers.changeProductQuantity(req.body).then(async (response) => {
        ...
        response.stock = await userHelpers.getStock(req.body.user)
        let stockValue = null;
        let quantity = null;
        if(response.stock[0].stock&&response.stock[0].quantity&& response.stock[0].stock >= response.stock[0].quantity){
            stockValue = response.stock[0].stock;
            quantity = response.stock[0].quantity;
            stockValue = stockValue-quantity;
        }
        response.stockValue = stockValue;
        // response.stock = stockValue
...
let shopProduct = async (req, res) => {
    ...
    let products = await productHelpers.shopAllProducts()
    let count = 0
    ...
    if (req.query.minimum) {
        ...
        products.forEach(product => {
            if (product.price >= minimum && products.price <= maximum) {
                arr.push(product)
            }
        });
        products = arr;
