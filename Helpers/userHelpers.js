...
module.exports = {
  ...
  blockUser: (id) => {
    return new Promise((resolve, reject) => {
      try {
        db.get()
          .collection(collection.USER_COLLECTION)
          .updateOne({ _id: objectId(id) }, { $set: { signupstatus: true } });
        resolve();
      } catch (err) {
        let error = {};
        error.message = "Something went wrong";
  ...
  getTotalAmount: (userId) => {
    try {
      ...
      res
        .status(500)
        .render("user/cartEmpty", { message: "please login or signup" });
    }
  },
  ...
  otpLogin: (otpDetails) => {
    return new Promise(async (resolve, reject) => {
      ...
      let user = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ number: otpDetails.number });
      if (user) {
  ...
  forgotPassword: (ForgotPasswordDetails) => {
    return new Promise(async (resolve, reject) => {
      let response = {};
      let registerednumber = await db
        .get()
        .collection(collection.USER_COLLECTION)
        .findOne({ number: ForgotPasswordDetails.registerednumber });
      if (registerednumber) {
  ...
  deleteWishProduct: (wishDetails) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.WISHLIST_COLLECTION)
        .updateOne(
          { _id: objectId(wishDetails.wishList) },
          {
